...
export const genderCategories = [
  ...
  // Configuración para Estado Civil
  export const maritalStatusCategories = [
  ...
  // Configuración para Nivel de Estudios
  ...
  // Configuración para Estrato
  export const stratumCategories = [
  ...
  // Configuración para Tipo de Vivienda
  ...
  // Configuración para Tipo de Cargo
  export const positionTypeCategories = [
  ...
  // Configuración para Tipo de Contrato
  export const contractTypeCategories = [
  ...
  // Configuración para Tipo de Salario
  export const salaryTypeCategories = [
...
export default function DadgraficoPersonal({datos}:{datos:any[]}) {
  ...
  const genderData = transformFormData(datos, 'sexo');
  const maritalStatusData = transformFormData(datos, 'estadoCivil');
  const educationData = transformFormData(datos, 'nivelEstudios');
  const stratumData = transformFormData(datos, 'estrato');
  const housingData = transformFormData(datos, 'tipoVivienda');
  const positionTypeData = transformFormData(datos, 'tipoCargo');
  const contractTypeData = transformFormData(datos, 'tipoContrato');
  const salaryTypeData = transformFormData(datos, 'tipoSalario');
...
/*
      ...
      nivelEstudios: educationCategories,f
      estrato: stratumCategories,c
      tipoVivienda: housingCategories,f
      tipoCargo: positionTypeCategories,f
...
// cosas a resaltar es que hay que hacer el const para cada cosa 
    return (
...
export const transformFormData = (data: any[], category: string) => {
    ...
    // Transformar a formato del gráfico
    return Object.entries(counts).map(([id, value]) => {
      const categoryInfo = categoryConfig.find(cat => cat.id === id);
      return {
        id,
        value,
        fill: categoryInfo?.color || 'hsl(0 0% 60%)'
      };
    });
  };
  
  // Función para obtener la configuración de categorías
