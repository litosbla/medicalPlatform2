...
import DadgraficoPersonal from "@/components/resultados/dadgraficopersonal";
...
export default function DashboardPrincipalPer({citaActual}: {citaActual: string}) {
    const [currenPersonales, setCurrentPersonales] = useState<Array<Schema["FormularioPersonales"]["type"]>>([]);
    const [isLoading, setIsLoading] = useState(false);

    const fetchInitialData = async () => {
        setIsLoading(true);
        try {
            const personalesData = await client.models.FormularioPersonales.list({
                filter: { citaId: { eq: citaActual } }
            });
            setCurrentPersonales(personalesData.data);
    ...
    <div className="w-full flex flex-col gap-4 p-4">
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-wrap w-full gap-4 justify-center mt-5">
                        ...
                        <DadgraficoPersonal datos={currenPersonales}/>
